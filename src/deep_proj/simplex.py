...
def plot_latent_simplex(model, loader, device,
                        model_type="gaussian", n_samples=None,
                        save_dir=None, model_name="Model",
                        cmap="tab10", point_size=50, alpha=0.7,
                        image_zoom=0.55):
    """
    ...
    for v_idx in range(latent_dim):
        ...
        ab = AnnotationBbox(imagebox, vertices[v_idx],
                            frameon=False, zorder=10)
        ax.add_artist(ab)

    ax.set_aspect("equal")
    ax.axis("off")
    plt.title(f"{model_name} – Latent Simplex")
